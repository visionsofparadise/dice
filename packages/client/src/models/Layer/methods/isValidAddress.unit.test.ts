...
describe("is address valid", () => {
	...
	beforeEach(() => {
		...
		layer.local = new Ipv4Address({
			ip: new Uint8Array([127, 0, 0, 1]),
			port: 3000,
		});

		layer.coordinators.add(
			new Ipv4Address({
		...
		layer.options.bootstrapAddresses = [
			new Ipv4Address({
	...
	it("rejects bootstrap address", () => {
		expect(isValidOverlayAddress(layer, layer.options.bootstrapAddresses[0]!)).toBe(false);
	});
});
