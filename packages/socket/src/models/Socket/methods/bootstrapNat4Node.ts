import { Socket } from "..";
import { Nat4Node } from "../../Node/Nat4";
import { ExternalAddressResult } from "./getExternalAddress";

export const bootstrapSocketNat4Node = (socket: Socket, externalAddressResult: ExternalAddressResult): void => {
	try {
		const { reflectNodeA, addressA, addressB } = externalAddressResult;

		if (addressA.toString() === addressB.toString()) return;

		socket.node = Nat4Node.create(
			{
				address: addressA,
